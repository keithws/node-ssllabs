...
/*
 * TODO don't use the global agent, refactor each http.get to use an options
 * object with the agent
 */
if (process.env.HTTPS_PROXY) {
	http.globalAgent = new ProxyAgent(process.env.HTTPS_PROXY);
}
...
module.exports.info = function (callback) {
	...
	getDataFromAPI("info", function (err, data) {
...
module.exports.getStatusCodes = function (callback) {
	...
	getDataFromAPI("getStatusCodes", function (err, data) {

		// keep a copy of the status codes in the api object
		api.statusCodes = data;
		callback(err, data);
...
module.exports.getRootCertsRaw = function (options, callback) {
	...
	if (isNaN(options.trustStore)) {
		...
		if (options.trustStore < 1 || options.trustStore > 5) {
...
module.exports.scan = function (options, callback) {
	...
	module.exports.info(function (err, info) {
		...
		if (info.currentAssessments < info.maxAssessments) {
			...
			setTimeout(
				module.exports.scan.bind(null, options, callback),
